...
def opt_multi_ilp_cplex(wps, E, S, rewards, weights, distance, hovering, num_drone, debug):
    ...
    # at least one edge must enter and exit to/from the depot
    model.add_constraint(model.sum(y[0, j, z] for j in range(1, M) for z in range(D)) == D)
    model.add_constraint(model.sum(y[j, 0, z] for j in range(1, M) for z in range(D)) == D)
    ...
    for i in range(N):
        ...
        for z in range(D):
            print("DRONE ", z)
            selected_x = [(i, j, z) for j in range(M) if sol.get_value(x[i, j, z]) >= 0.99]
            total_x_selected = total_x_selected + selected_x
            if debug:
                print("Waypoints: {}".format(selected_x))
            for j in range(M):
    ...
    for z in range(D):
        ...
        for l in range(M):
            ...
            total_y_selected = total_y_selected[z].append(selected_y)
            if debug:
                print("Edges: {}".format(selected_y))
            for j in range(M):
